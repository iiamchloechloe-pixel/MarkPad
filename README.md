...
- 关闭、退出和系统打开其他文件时检查未保存更改；编辑内容自动备份为本机草稿，下次启动恢复。
- 保存前检查磁盘内容；发现外部修改时转为另存副本，保存失败保留编辑状态。
- 查找替换统一在源码模式定位，关闭查找后恢复原编辑模式；支持字面替换及撤销。
- `⌘P` 快速打开最近文件与当前文件夹中的文档，支持文件名、路径过滤及方向键选择。
- 工具栏「写作设置」或 `⌘,` 调整字号、行距、正文宽度，设置自动记忆。
- 回归验证：`node_modules/.bin/electron tests/smoke.cjs`（使用独立临时配置）。

草稿备份不等同于自动覆盖原始文件；写回 `.md` 仍使用保存操作。全文搜索、自动保存原文件及图片导出改进留待后续版本。
...
- **导出**：导出 PDF、导出 HTML
- **多种主题**：点工具栏 🅰 下拉直接选 —— 默认 / 护眼 / 棕褐 / 打字机 / 夜读（`⌘⇧P` 切下一个）；外观可跟随系统日 / 夜（`⌘D` 循环浅 / 深 / 跟随）
